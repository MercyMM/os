...
{ 
	...
	/*
	//用于保存预扫描的起始环境，以便预扫描返回时恢复执行环境!
	old_seek = 源码扫描指针; 
	old_scan = scan;
	old_next = next; 
	 */
	af_stk_top -= 1;
	*af_stk_top = FALSE; //初始设置为FALSE

	old_pre_scan = pre_scan; 
	//要点：1全局变量pre_scan的初值为FALSE!2所有相关函数的语义动作前，都进行pre_scan==TRUE检查!
	//标志当前执行的语义动作只是预扫描，不是玩真的
	pre_scan = TRUE;
	...
	pre_scan = old_pre_scan;//恢复原预扫描标志

	/*
	//恢复预扫描前的执行环境
	源码扫描指针= old_seek;
	scan = old_scan;
	next = old_next;//恢复扫描起点
	*/
	printf("roll back\n");
...
{
	TEST_OUTPUT();
	if(identify_assign_exp() != TRUE ) { 
		printf("First extent\n");
		conditional_expression(); //1   
		return;
	} else {
		printf("second extent\n");
		if(is_first(NTS_UNARY_EXP, scan)) { //2
			unary_expression();
			assignment_operator();
			assignment_expression();//右结合语义，用递归实现
		} else
			output_error("assignment_expression的first字符出错");
...
int unary_expression()
{
	...
	if(is_first(NTS_POSTFIX_EXP, scan)) {
		...
		if(is_follow(NTS_UNARY_EXP, scan)) {
			if(pre_scan && is_assignment_op(scan) == TRUE) {
				*af_stk_top = TRUE;       //unary-expression 是赋值表达式左部！
				return;
			} else if (pre_scan)
				return;//FALSE出口
		}
		else
			output_error("unary-expression之后的字符出错!");
	}else if(scan == TS_INC || scan == TS_DEC) {
		skip(scan);
		unary_expression( );
		if(is_follow(NTS_UNARY_EXP, scan)) {
			if(pre_scan && is_assignment_op(scan) == TRUE) {
		...
		skip(TS_SIZEOF);
		if(scan != TS_LPAREN && is_first(NTS_UNARY_EXP, scan))
			unary_expression();
		else if(scan == TS_LPAREN && is_first(NTS_TP_NAME, next)) {
			skip(TS_LPAREN);
			type_name();
			skip(TS_RPAREN);
			...
			output_error("在cast-expression中出错");

		if(is_follow(NTS_UNARY_EXP, scan)) 
			return; //情况5一定不是赋值表达式左部!无论预扫描否，返回FALSE
		else
			output_error("sizeof  unary-expression的follow字符出错！");
	} else if(pre_scan)
		return;//FALSE出口
	else
		output_error("unary_expression的起始字符出错！");//通常错误出口
}
/*postfix_expression:
	 primary_expression 
